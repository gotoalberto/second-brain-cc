...
- **Forcing a task that belongs to another machine is refused**, with a message naming the owner
  and this machine's aliases, and an explicit override flag for when you really mean it.
- **The machine registry (`_bin/machines.py`) records the Claude account** each machine is signed
  into, and collapses aliases of the same machine when listing. Duplicate entries are reported,
