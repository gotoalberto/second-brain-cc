...
# database, Google accounts, object storage, alert email, the MCP server, scheduled jobs, CLI-agent
# routines). Nothing is installed without a yes; answers are remembered, so it is safe to re-run.
#
...
if [ ! -t 0 ]; then
  ...
  echo "Unattended machines and CI can record every step as declined instead:"
