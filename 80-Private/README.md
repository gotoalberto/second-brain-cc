...
Use it for the things you want the vault to hold but not to sync: raw meeting notes before you sanitise them, personal reminders, scratch thinking, a draft that mentions a real name or host you haven't abstracted yet, or a pointer to a secret (store the secret itself in the local KeePass database with `_bin/kp.py` and reference it with a `kp://` link, never inline).
