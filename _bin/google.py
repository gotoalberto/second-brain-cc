...
OAuth flow and the REST calls are the standard library.
  ...
  google.py auth --account NAME [--no-browser] [--timeout S]
      one-off consent: opens Google's page, waits on 127.0.0.1:<port>, stores the refresh token
  google.py token --account NAME
      print a fresh access token (for a manual curl)
  google.py api --account NAME URL [--method GET|POST|PUT|PATCH|DELETE] [--body-file FILE]
  ...
  google.py send --account NAME --to ADDRESS --subject S [--body-file FILE] [--html]
      ...
      stdin or --body-file. Every delivered message appends one JSON line (time, account, to, subject,
...
delivered), 2 asked wrongly (usage, an unknown account). token, api and send never prompt: they read
KeePass headless (arm its cache with `kp.py unlock`), so they are safe in scheduled jobs.
...
def make_parser():
    ...
    sub = ap.add_subparsers(dest="cmd", metavar="{accounts,add,auth,token,api,send}", parser_class=_Parser)
